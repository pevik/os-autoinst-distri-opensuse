...
sub run {
    ...
    my $klog_stamp_base = "echo 'OpenQA::run_ltp.pm: Starting $test->{name}' > /dev/";
    ...
    if (is_serial_terminal) {
        script_run($klog_stamp_base . "kmsg");
        # SLE11-SP4 don't support ignore_loglevel, due that stamp is not printed in console
        script_run($klog_stamp_base . $serialdev) if is_sle('<12');
