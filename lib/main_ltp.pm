...
use version_utils qw(is_released is_sle);
use main_common qw(boot_hdd_image get_ltp_tag load_ayinst_tests load_boot_tests load_bootloader_s390x load_inst_tests load_reboot_tests loadtest);
use 5.018;
...
sub shutdown_ltp {
    load_kernel_test('proc_sys_dump') if get_var('PROC_SYS_DUMP');
    load_kernel_test('shutdown_ltp', @_);
}
...
sub parse_openposix_runfile {
    ...
    while (my $line = <$rfile>) {
        ...
        if ($line =~ m/$cmd_pattern/ && !($line =~ m/$cmd_exclude/)) {
            ...
            load_kernel_test('run_ltp', name => $test->{name}, run_args => $tinfo);
        }
    }
}
...
sub load_kernel_test_from_runtest_file {
    ...
    load_kernel_test('boot_ltp', run_args => testinfo($test_result_export));
    if (get_var('LTP_COMMAND_FILE') =~ m/ltp-aiodio.part[134]/) {
        load_kernel_test 'create_junkfile_ltp';
    }
...
# Replace load_kernel_test_from_runtest_file with this to stress test reverting to
# snapshots
sub stress_snapshots {
    ...
    for (my $i = 0; $i < $count / 2; $i++) {
        ...
        load_kernel_test('run_ltp');
        load_kernel_test('write_random');
    }
}
...
sub load_kernel_tests {
    ...
    load_kernel_test "../installation/bootloader" if is_pvm;

    if (get_var('INSTALL_LTP')) {
        if (get_var('INSTALL_KOTD')) {
            load_kernel_test 'install_kotd';
        }
        elsif (get_var('CHANGE_KERNEL_REPO') ||
            get_var('CHANGE_KERNEL_PKG') ||
            get_var('ASSET_CHANGE_KERNEL_RPM')) {
            load_kernel_test 'change_kernel';
        }
        if (get_var('FLAVOR', '') =~ /Incidents-Kernel/) {
            load_kernel_test 'update_kernel';
        }
        load_kernel_test 'install_ltp';
        ...
        load_kernel_test_from_runtest_file();
    }
        ...
        load_kernel_test 'boot_ltp';
        load_kernel_test 'qa_test_klp';
    }
    elsif (get_var('VIRTIO_CONSOLE_TEST')) {
        load_kernel_test 'virtio_console';
        load_kernel_test 'virtio_console_long_output';
    }
    elsif (get_var('NVMFTESTS')) {
        boot_hdd_image();
        load_kernel_test 'nvmftests';
    }
        ...
        load_kernel_test "trinity";
    } elsif (get_var('NUMA_IRQBALANCE')) {
        boot_hdd_image();
        load_kernel_test 'numa_irqbalance';
    }

    if (check_var('BACKEND', 'svirt') && get_var('PUBLISH_HDD_1')) {
        load_kernel_test '../shutdown/svirt_upload_assets';
    }
}
